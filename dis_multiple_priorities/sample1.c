...
TaskHandle_t btask1_m_handle;
TaskHandle_t btask2_m_handle;
...
void
btask1_m(__unused void *params)
{
    int last_core_id = -1;
    int cur_core_id;
    int c;

    printf("btask1_m : Start!\n");
    printf("btask1_m : Press any key to resume otask_h\n");

    message = 0;

    while(true) {
        printf("btask1_m : running on core %d\n", GetCoreID());
...
void
btask2_m(__unused void *params)
{
    int last_core_id = -1;
    int cur_core_id;

    printf("btask2_m : Start!\n");
    
    while(true) {
        printf("btask2_m : running on core %d\n", GetCoreID());
...
main( void )
{
    ...
    xTaskCreate(btask1_m, "btask1_m", TASK_STACK_SIZE, NULL, MID_TASK_PRIORITY, &btask2_m_handle);
    xTaskCreate(btask2_m, "btask2_m", TASK_STACK_SIZE, NULL, MID_TASK_PRIORITY, &btask2_m_handle);
